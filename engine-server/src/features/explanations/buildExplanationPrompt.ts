...
export function buildExplanationPrompt(
  ...
  const instructions = [
    ...
    '- 不正解手を書くときは，まず contrast_features.ownStrengths を見る',
    '- ownStrengths がある場合は，その手に一応ある良さを1つ本文に入れる',
    '- 次に contrast_features.contrastPhrases で，正解手に劣る理由を書く',
    '- 不正解手は「一応あるもの + それでも足りない理由」を基本形にする',
    '- contrast_features.confidence が medium の場合は，ownStrengths と contrastPhrases を主材料にしてよい',
    ...
    '- 具体factがある不正解手は，「攻め筋が消える」「手が遅い」「厳しい狙いがない」のように理由を具体化する',
    '- 「攻め味が弱い」「攻め味が薄い」は，他に具体phraseがない最後の場合だけ使う',
    '- 「攻め筋が消える」「攻めがなくなる」のような強い断定は，featuresで明確に確認できる場合だけ使う',
    '- 根拠が弱い場合は，「後続の攻めが弱い」「正解手ほど攻めが続かない」に留める',
    '- 「見込みがある」「可能性がある」のような曖昧表現は使わない',
