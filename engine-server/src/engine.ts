...
export class ShogiEngine {
  ...
  constructor(enginePath?: string, evalDir?: string) {
    ...
    } else if (envPath && envPath.length > 0) {
    ...
    console.log(
      `[engine] default tuning: hash=${this.tuning.hashMb}MB threads=${this.tuning.threads} pvInterval=${this.tuning.pvIntervalMs} multipv=${this.tuning.multipv} ownBook=${this.tuning.ownBook}`,
    );
  }
